 ...
 *   `buildAbilityFromScopes(getAllScopesForOrgRole(role))`.
 ...
 *   covered by `orgRoleToScopeMapping.test.ts` and
 *   `orgSystemRoleRouting.test.ts`). `serviceAccountAbility.test.ts`'s
 *   existing assertions (the "11 cases" delegation-permission-footprint
 *   suite included) exercise this same path and stay green.
 *
 * Mirrors `orgSystemRoleRouting.test.ts`'s structure and rationale.
 */
