 ...
 *   through, not re-proving role/scope behavioral equivalence
 *   (`roleToScopeMapping.test.ts` and `orgRoleToScopeMapping.test.ts`
 *   already cover that).
 *
 * The org layer here is held at the minimal `member` tier and built the
 * same way, so the "expected" reference constructs both layers via
