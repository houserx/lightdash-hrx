 ...
 * role_uuid, see `resolveEffectiveOrgRoleUuid`) over the literal
 * `orgRoleToScopeMapping.ts` map -- the same DB-first, literal-map-fallback
 * resolution `getUserAbilityBuilder` uses for ability building, applied
 * here to the confused-deputy grant check instead.
 */
